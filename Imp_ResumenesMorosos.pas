...
type
  TImp_ResumenMorosos = class(TForm)
    ToolBar1: TToolBar;
    ...
    CB0: TCheckBox;
    JvBitBtn2: TJvBitBtn;
    RichEdit1: TRichEdit;    procedure FormActivate(Sender: TObject);
...
var
  Imp_ResumenMorosos: TImp_ResumenMorosos;
...
procedure TImp_ResumenMorosos.FormActivate(Sender: TObject);
begin
...
procedure TImp_ResumenMorosos.FormCreate(Sender: TObject);
begin
gh:=false;
end;

procedure TImp_ResumenMorosos.Coloca1(nro:byte);
var t,y:integer;
...
if nro=12 then begin
  ...
  if IdCombos[1,TPOOBJ.ItemIndex] = '3' then begin   ///mantenimiento
   if cb1.Checked  then txtw:='SELECT COUNT(*) FROM LasAccionesResumenMorosos WHERE (cantmesesdeuda >= ' + trim(domi.Text) + ') and inhumados > 0'
    else txtw:='SELECT COUNT(*) FROM LasAccionesResumenMorosos WHERE (cantmesesdeuda >= ' + trim(domi.Text) + ') and inhumados = 0 ';
  end;
  if IdCombos[1,TPOOBJ.ItemIndex] = '1' then begin //ventas
   if cb1.Checked  then txtw:='SELECT COUNT(*) FROM LasAccionesResumenMorosos WHERE (DeudaVenta <> ''0.00'') AND inhumados > 0'
    else txtw:='SELECT COUNT(*) FROM LasAccionesResumenMorosos WHERE (DeudaVenta <> ''0.00'') AND inhumados = 0';
  end;
  if IdCombos[1,TPOOBJ.ItemIndex] = '2' then begin //servicios
   if cb1.Checked  then txtw:='SELECT COUNT(*) FROM LasAccionesResumenMorosos WHERE (DeudaServicio <> ''0.00'') AND inhumados > 0'
    else txtw:='SELECT COUNT(*) FROM LasAccionesResumenMorosos WHERE (DeudaServicio <> ''0.00'') AND inhumados = 0';
  end;
  if not Funciones.Listar(txtw,DtComboReg) then
     messagedlg('ERROR al realizar Transac-SQL. Consulte con Administrador',mtError,[mbok],0);
  Funciones.Free;
...
procedure TImp_ResumenMorosos.JvBitBtn1Click(Sender: TObject);
begin
Close;
end;

procedure TImp_ResumenMorosos.DESDEKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TImp_ResumenMorosos.HASTAKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TImp_ResumenMorosos.HASTAExit(Sender: TObject);
var t,u:integer;
begin
t:=0;u:=0;
try
...
procedure TImp_ResumenMorosos.DBGrid1CellClick(Column: TColumn);
begin
...
procedure TImp_ResumenMorosos.DataSource1DataChange(Sender: TObject;
  Field: TField);
begin
...
procedure TImp_ResumenMorosos.RvSPrint(Sender: TObject);
...
procedure TImp_ResumenMorosos.IMPRIMEClick(Sender: TObject);
var gf:integer;
    ps:string;
begin
...
procedure TImp_ResumenMorosos.resumcobClick(Sender: TObject);
begin
Coloca1(5);
rvc.Execute;
end;

procedure TImp_ResumenMorosos.RvCPrint(Sender: TObject);
...
var pso:boolean;
   ...
   begin
     with RvC do begin
      ...
      BaseReport.SetFont('Arial Black',9);
      BaseReport.FontColor:=clBlack;
      if cb3.Checked then BaseReport.PrintXY(posfx+80,posfy+24,'Cantidad de ' +
       'Meses de Deuda: ' + trim(operador.Text ) + ' ' + trim(domi.Text ))
       else BaseReport.PrintXY(posfx+80,posfy+24,'Acciones SIN DEUDA');
...
procedure TImp_ResumenMorosos.COBRADORExit(Sender: TObject);
begin
...
procedure TImp_ResumenMorosos.COBRADORKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TImp_ResumenMorosos.DBGrid1DrawDataCell(Sender: TObject;
...
procedure TImp_ResumenMorosos.PERIODOKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then begin
  Key := #0;
...
procedure TImp_ResumenMorosos.PERIODOExit(Sender: TObject);
begin
...
procedure TImp_ResumenMorosos.BitBtn1Click(Sender: TObject);
begin
 labelca.Caption:='...';
 labelcm.Caption:='...';
 labelpm.Caption:='...';
...
procedure TImp_ResumenMorosos.BitBtn2Click(Sender: TObject);
var    txtw:widestring;
var     a:integer;
begin
   totaldeudagral:=0;
   ...
   + ',' + CHR(39) + txtfecha + CHR(39)+ ',null';  //+ ',' + CHR(39) + txtfecha + CHR(39)+ ',' + trim(domi.Text);
   PROCESO2.Open;
   if PROCESO2.Fields[0].AsString = 'OK' then begin
    ...
    Funciones:=TFuncionesVarias.Create(nil);
    Funciones.ConnectionString:=modulo2.Conexion;
    if not cb0.Checked then  begin
     if IdCombos[1,TPOOBJ.ItemIndex] = '3' then begin   ///mantenimiento
      if not cb1.Checked  then begin  // todas las acciones
       if cb3.Checked then begin // con filtro de cantidad de meses
       txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,Sector,parcela,plaza,fechacesion,'  //5
       +'inhumados,gestor,cuif,primertitular,telefono,DomicilioTit,domcobro,DeudaMantenimiento,MesesDeudaMantenimiento,' //14
       +'convert(nvarchar(25),MesesDeudaMantenimiento)+ '' - $ '' +convert(nvarchar(25),DeudaMantenimiento) as MesesDeuda,' //15
       +'nrodni,DeudaMantenimiento + deudaventa + deudaservicio + DeudaConsolidado as DeudaTotal,' //17
       +'deudaventa, deudaservicio ,DeudaConsolidado ,' //20
       +'convert(nvarchar(8),nroaccion) +'' Sec.'' + Sector + ''P:''+ convert(nvarchar(8),parcela) as dato' //21
       +' FROM AccionesDeudoras (0) where (MesesDeudaMantenimiento' + trim(operador.Text) + ' ' + trim(domi.Text) + ')'
       +' AND (MesesDeudaMantenimiento > 1) order by 1';

       {txtw:='select row_number() over (order by ger.NroAccion asc) as Orden,ger.nroaccion,ger.sector,ger.parc,ger.plaza,'
       +'ger.fechasecion,ger.inhumados,ger.gestor,ger.cuif,ger.primertitular,ger.telefono,ger.DomicilioTit,ger.domcobro,'
       +'ger.observacion,ger.cantmesesdeuda,ger.MesesDeuda,ger.nrodni from (SELECT nroaccion,sector,parc,plaza,fechasecion,'
       +'inhumados,gestor,cuif,primertitular,telefono,''Bº '' + barrio + '' - '' + domcilio as DomicilioTit,domcobro,'
       +'observacion,cantmesesdeuda,convert(nvarchar(3),cantmesesdeuda) + '' - $ '' + observacion as MesesDeuda,nrodni '
       +' FROM LasAccionesResumenMorosos  WHERE  (cantmesesdeuda > 1)) ger '
       +' where (ger.cantmesesdeuda ' + trim(operador.Text) + ' ' + trim(domi.Text) + ') order by 1'; }
       end
       else begin
       txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,Sector,parcela,plaza,fechacesion,'
       +'inhumados,gestor,cuif,primertitular,telefono,DomicilioTit,domcobro,DeudaMantenimiento,MesesDeudaMantenimiento,'
       +'convert(nvarchar(25),MesesDeudaMantenimiento)+ '' - $ '' +convert(nvarchar(25),DeudaMantenimiento) as MesesDeuda,'
       +'nrodni,DeudaMantenimiento +deudaventa + deudaservicio + DeudaConsolidado  as DeudaTotal,'
       +'deudaventa,deudaservicio,DeudaConsolidado,convert(nvarchar(8),nroaccion) +'' Sec.'' + Sector + ''P:''+ convert(nvarchar(8),parcela) as dato'
       +' FROM AccionesDeudoras (0) where (MesesDeudaMantenimiento > 1) order by 1';
       ...
       +'inhumados,gestor,cuif,primertitular,telefono,''Bº '' + barrio + '' - '' + domcilio as DomicilioTit,domcobro,'
       +'observacion,cantmesesdeuda,convert(nvarchar(3),cantmesesdeuda) + '' - $ '' + observacion as MesesDeuda,nrodni '
       +' FROM LasAccionesResumenMorosos  WHERE  (cantmesesdeuda > 1)) ger order by 1'; }
       end;
      end;
      if cb1.Checked then begin // SOLO CON INHUMADOS
       if cb3.Checked then begin // con filtro de cantidad de meses
       txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,Sector,parcela,plaza,fechacesion,'
       +'inhumados,gestor,cuif,primertitular,telefono,DomicilioTit,domcobro,DeudaMantenimiento,MesesDeudaMantenimiento,'
       +'convert(nvarchar(25),MesesDeudaMantenimiento)+ '' - $ '' +convert(nvarchar(25),DeudaMantenimiento) as MesesDeuda,'
       +'nrodni,DeudaMantenimiento +deudaventa + deudaservicio + DeudaConsolidado  as DeudaTotal,'
       +'deudaventa,deudaservicio,DeudaConsolidado,convert(nvarchar(8),nroaccion) +'' Sec.'' + Sector + ''P:''+ convert(nvarchar(8),parcela) as dato'
       +' FROM AccionesDeudoras (1) where (MesesDeudaMantenimiento' + trim(operador.Text) + ' ' + trim(domi.Text) + ')'
       +' AND (MesesDeudaMantenimiento > 1)  order by 1';
       ...
       +'observacion,cantmesesdeuda,convert(nvarchar(3),cantmesesdeuda) + '' - $ '' + observacion as MesesDeuda,nrodni '
       +' FROM LasAccionesResumenMorosos  WHERE  (cantmesesdeuda > 1)) ger '
       +' where (ger.cantmesesdeuda ' + trim(operador.Text) + ' ' + trim(domi.Text) + ') and ger.inhumados > 0 order by 1';}
       end
       else begin
       txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,Sector,parcela,plaza,fechacesion,'
       +'inhumados,gestor,cuif,primertitular,telefono,DomicilioTit,domcobro,DeudaMantenimiento,MesesDeudaMantenimiento,'
       ...
       +'nrodni,DeudaMantenimiento +deudaventa + deudaservicio + DeudaConsolidado as DeudaTotal,'
       +'deudaventa,deudaservicio,DeudaConsolidado,convert(nvarchar(8),nroaccion) +'' Sec.'' + Sector + ''P:''+ convert(nvarchar(8),parcela) as dato'
       +' FROM AccionesDeudoras (1) where (MesesDeudaMantenimiento > 1) order by 1';
       ...
       +'inhumados,gestor,cuif,primertitular,telefono,''Bº '' + barrio + '' - '' + domcilio as DomicilioTit,domcobro,'
       +'observacion,cantmesesdeuda,convert(nvarchar(3),cantmesesdeuda) + '' - $ '' + observacion as MesesDeuda,nrodni '
       +' FROM LasAccionesResumenMorosos  WHERE  (cantmesesdeuda > 1)) ger where ger.inhumados > 0 order by 1';  }
       end;
      end;
     end;
    end
     ...
     if IdCombos[1,TPOOBJ.ItemIndex] = '3' then begin   ///mantenimiento
      //if IdCombos[1,TPOOBJ.ItemIndex] = '0' then begin   ///solo sin deuda
      if not cb1.Checked  then begin  // todas las acciones
       if cb3.Checked then begin // con filtro de cantidad de meses
       txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,Sector,parcela,plaza,fechacesion,'  //5
       +'inhumados,gestor,cuif,primertitular,telefono,DomicilioTit,domcobro,DeudaMantenimiento,MesesDeudaMantenimiento,' //14
       ...
       txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,Sector,parcela,plaza,fechacesion,'
       +'inhumados,gestor,cuif,primertitular,telefono,DomicilioTit,domcobro,DeudaMantenimiento,MesesDeudaMantenimiento,'
       +'convert(nvarchar(25),MesesDeudaMantenimiento)+ '' - $ '' +convert(nvarchar(25),DeudaMantenimiento) as MesesDeuda,nrodni,'
       +'DeudaMantenimiento +deudaventa + deudaservicio + DeudaConsolidado  as DeudaTotal,deudaventa,deudaservicio,'
       +'DeudaConsolidado,convert(nvarchar(8),nroaccion) +'' Sec.'' + Sector + ''P:''+ convert(nvarchar(8),parcela) as dato'
       +' FROM AccionesDeudoras (0) where (MesesDeudaMantenimiento <= 1) order by 1';//
      ...
      if cb1.Checked then begin // SOLO CON INHUMADOS
       ...
       +'inhumados,gestor,cuif,primertitular,telefono,DomicilioTit,domcobro,DeudaMantenimiento,MesesDeudaMantenimiento,'
       +'convert(nvarchar(25),MesesDeudaMantenimiento)+ '' - $ '' +convert(nvarchar(25),DeudaMantenimiento) as MesesDeuda,nrodni,'
       +'DeudaMantenimiento +deudaventa + deudaservicio + DeudaConsolidado as DeudaTotal,deudaventa,deudaservicio,DeudaConsolidado,'
       +'convert(nvarchar(8),nroaccion) +'' Sec.'' + Sector + ''P:''+ convert(nvarchar(8),parcela) as dato'
       +' FROM AccionesDeudoras (1) where (MesesDeudaMantenimiento <= 1) order by 1';
    ...
    if IdCombos[1,TPOOBJ.ItemIndex] = '1' then begin   ///ventas
     if not cb1.Checked  then begin  // todas las acciones
      ...
      + 'gestor,cuif,primertitular,telefono,''Bº '' + barrio + '' - '' + domcilio as DomicilioTit,domcobro,'
      + 'DeudaVenta,cantmesesdeuda,'' - $ '' + DeudaVenta,nrodni '
      + ' FROM LasAccionesResumenMorosos  WHERE (DeudaVenta <> ''0.00'') order by 1';
     end;
     if cb1.Checked then begin // CON INHUMADOS
      txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,sector,parc,plaza,fechasecion,inhumados,'
      + 'gestor,cuif,primertitular,telefono,''Bº '' + barrio + '' - '' + domcilio as DomicilioTit,domcobro,'
      + 'DeudaVenta,cantmesesdeuda,'' - $ '' + DeudaVenta,nrodni '
      + ' FROM  LasAccionesResumenMorosos  WHERE (DeudaVenta <> ''0.00'') and inhumados > 0 order by 1';
     end;
    end;
    ...
    if IdCombos[1,TPOOBJ.ItemIndex] = '2' then begin   ///servicios
     if not cb1.Checked  then begin  //  todas las acciones
      txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,sector,parc,plaza,fechasecion,inhumados,'
      + 'gestor,cuif,primertitular,telefono,''Bº '' + barrio + '' - '' + domcilio as DomicilioTit,domcobro,'
      + 'DeudaServicio,cantmesesdeuda,'' - $ '' + DeudaServicio,nrodni '
      + ' FROM LasAccionesResumenMorosos  WHERE (DeudaServicio <> ''0.00'') order by 1';
     end;
     if cb1.Checked then begin // CON INHUMADOS
      txtw:='SELECT row_number() over (order by NroAccion asc) as Orden,nroaccion,sector,parc,plaza,fechasecion,inhumados,'
      + 'gestor,cuif,primertitular,telefono,''Bº '' + barrio + '' - '' + domcilio as DomicilioTit,domcobro,'
      + 'DeudaServicio,cantmesesdeuda,'' - $ '' + DeudaServicio,nrodni '
      + ' FROM  LasAccionesResumenMorosos  WHERE (DeudaServicio <> ''0.00'') and inhumados > 0 order by 1';
     end;
    end;
    ///
    ...
    if not DataDeudas.IsEmpty then begin
     ...
     try
      ...
      begin
       totalmeses := totalmeses + dbgrid1.datasource.dataset.fieldbyname('MesesDeudaMantenimiento').asfloat;
       totaldeudagral:= totaldeudagral + dbgrid1.datasource.dataset.fieldbyname('DeudaTotal').asfloat;
    ...
    LABELCM.Caption :=IntToStr(DataDeudas.RecordCount);
    //porcen:= totregm *100 / strtoint(cantreg);
    porcen:= DataDeudas.RecordCount *100 / strtoint(cantreg);
...
procedure TImp_ResumenMorosos.BitBtn3Click(Sender: TObject);
var t,y:integer;
   txtw:widestring;
begin
 PROCESO3.Close;
...
procedure TImp_ResumenMorosos.Button3Click(Sender: TObject);
var gf:integer;
    ps:string;
begin
...
procedure TImp_ResumenMorosos.PageControl1Change(Sender: TObject);
begin
if PageControl1.ActivePageIndex = 1  then begin
      fechad.SetFocus;
end;
...
procedure TImp_ResumenMorosos.FECHADExit(Sender: TObject);
var kl:Tdatetime;
begin
try
...
procedure TImp_ResumenMorosos.FECHAHExit(Sender: TObject);
var kl:Tdatetime;
begin
...
procedure TImp_ResumenMorosos.FECHADKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then begin
  Key := #0;
...
procedure TImp_ResumenMorosos.FECHAHKeyPress(Sender: TObject;
  var Key: Char);
begin
 if key = #13 then begin
  if strtodate(fechah.Text) < strtodate(fechad.Text) then begin
...
procedure TImp_ResumenMorosos.TPOOBJExit(Sender: TObject);
begin
 if TPOOBJ.ItemIndex < 0 then begin
  ...
  if IdCombos[1,TPOOBJ.ItemIndex] = '3'  then domi.Enabled:=true
   else domi.Enabled:=false;
 end;
end;

procedure TImp_ResumenMorosos.TPOOBJKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then begin
  Key := #0;
...
procedure TImp_ResumenMorosos.DOMIExit(Sender: TObject);
begin
...
procedure TImp_ResumenMorosos.DOMIKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then begin
  Key := #0;
  BitBtn1.SetFocus;
...
procedure TImp_ResumenMorosos.RvNOTAPrint(Sender: TObject);
...
procedure TImp_ResumenMorosos.BitBtn4Click(Sender: TObject);
var t:integer;
    txtw:widestring;
begin
 Funciones:=TFuncionesVarias.Create(nil);
 ...
 try
  ...
  begin
   ...
   +',CantPeriodosMantenimiento,Observaciones,NroDni) VALUES (''Noviembre2018'','
...
procedure TImp_ResumenMorosos.operadorKeyPress(Sender: TObject;
  var Key: Char);
begin
if key = #13 then begin
  Key := #0;
...
procedure TImp_ResumenMorosos.operadorExit(Sender: TObject);
begin
...
procedure TImp_ResumenMorosos.CB3Click(Sender: TObject);
begin
...
procedure TImp_ResumenMorosos.CB3KeyPress(Sender: TObject; var Key: Char);
begin
  if cb3.Checked then operador.SetFocus
    else bitbtn1.SetFocus;
end;

procedure TImp_ResumenMorosos.CB0Click(Sender: TObject);
begin
 IF CB0.Checked then begin
  cb3.Checked :=false;
  operador.Enabled  :=false;
  domi.Enabled :=false;
  cb1.SetFocus ;
...
procedure TImp_ResumenMorosos.JvBitBtn2Click(Sender: TObject);
var gf,t,y:integer;
