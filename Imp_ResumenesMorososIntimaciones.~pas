...
begin
  try
   ...
   if PageControl1.ActivePageIndex = 0  then begin
    ...
    RvC.BaseReport.PrintXY(posfx+120,posfy+10,'Cantidad Mínima de Meses de Deuda: ' + trim(domi.Text ));
...
begin
   ...
   if PROCESO2.Fields[0].AsString = 'OK' then begin
      ...
      if cb2.Checked and cb1.Checked then begin
       ...
       //+'case when nivelTitularidad = 1  then ''1º Tit - ''  + Titular '
       //+'when nivelTitularidad = 2  then ''2º Tit - '' + Titular else ''3º Tit - '' + Titular end as Titular, '
       //+'telefono,domicilio,null,observacion,cantmesesdeuda,detalle,dni FROM AccionesParaResumenConTodosTitulares (0)'
      end;
