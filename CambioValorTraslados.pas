...
var
  CamValorTraslado: TCamValorTraslado;
...
procedure TCamValorTraslado.FormCreate(Sender: TObject);
begin
gh:=false;
end;

procedure TCamValorTraslado.FormActivate(Sender: TObject);
begin
...
procedure TCamValorTraslado.Coloca1(nro:byte);
var t,y:integer;
    txt,sec:string;
    txtw:widestring;
...
begin
 if nro=1 then begin
  ...
  txtw:='SELECT Id,Descripcion,Fechavigencia,ValorKm,Activo,Iva,ImporteTotal,ImporteFijo FROM ParcelasServiciosTrasladosOrigen order by id desc'; // WHERE (Activo = 1)' ;
 ...
 if nro=2 then begin
  ...
  DataSource1.DataSet:=Dataset;
  ParcelasMantenimientoPrecios.ConnectionString:=modulo2.Conexion;
  txtw:='SELECT Id,Descripcion,Fechavigencia,ValorKm,Activo,Iva,ImporteTotal,ImporteFijo FROM ParcelasServiciosTrasladosOrigen order by id asc'; 
  if not ParcelasMantenimientoPrecios.Listar(txtw,DataSet) then
   messagedlg('ERROR al realizar Transac-SQL. Consulte con Administrador',mtError,[mbok],0);
  dbgrid1.Columns[0].Width:=30;
  dbgrid1.Columns[1].Width:=150;
...
procedure TCamValorTraslado.SALEClick(Sender: TObject);
begin
Close;
end;

procedure TCamValorTraslado.GUARDAClick(Sender: TObject);
var gf,t:integer;
var m,a:boolean;
var txtw:widestring;
begin
 gf:=messagedlgpos('Desea Realmente Incoporar un Nuevo Valor del Traslado?...',mtWarning,[
             mbok,mbCancel],0,screen.Width div 3,screen.Height div 3);
 if gf = 1 then begin
  PROCESO1.Close;
  PROCESO1.CommandText:='exec SPC_TiposServiciosTrasladosInsertar ' + chr(39) + trim(edit1.Text) + chr(39) + ','
  + trim(imp.Text) + ',' + chr(39) + trim(fecha.Text) + chr(39);
  PROCESO1.Open;
  if PROCESO1.Fields[0].AsString = 'OK' then begin
   messagedlg('EXITO al Guardar el VALOR del Traslado...',mtConfirmation,[mbok],0);
   Coloca1(1);
   guarda.Enabled :=false;
   grupovalor.Enabled:=false;
  end
  else begin
   messagedlg('Error al Guardar el VALOR del Traslado. Consulte con un Administrador',mtError,[mbok],0);
  end;
 end
 else begin
...
procedure TCamValorTraslado.JvBitBtn3Click(Sender: TObject);
begin
close;
end;

procedure TCamValorTraslado.DBGrid1CellClick(Column: TColumn);
var gf:integer;
begin
 if not (Dataset.IsEmpty) then begin
  if (trim(DBGrid1.Fields[4].Text) = 'False')  then begin
...
procedure TCamValorTraslado.DBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
 if (trim(DBGrid1.Fields[4].Text) = 'True')  then begin
...
procedure TCamValorTraslado.FECHAExit(Sender: TObject);
var fec:Tdatetime;
begin
  try
...
procedure TCamValorTraslado.FECHAKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TCamValorTraslado.impKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then begin
    Key := #0;
    guarda.SetFocus;
...
procedure TCamValorTraslado.impExit(Sender: TObject);
begin
 if (trim(imp.Text)='') or (trim(imp.Text)='0') then imp.Text:='00.00';
 if trim(imp.Text)='00.00' then begin
  messagedlg('ERROR!! EL Importe del Traslado NO puede ser Cero(0)!!',mterror,[mbok],0);
  imp.SetFocus;
 end;
end;

procedure TCamValorTraslado.JvBitBtn1Click(Sender: TObject);
begin
RVS1.Execute;
end;

procedure TCamValorTraslado.RvS1Print(Sender: TObject);
...
var sd,sd1,posn,posw:double;
   ...
   begin
    with RvS1 do begin
     ...
     BaseReport.FontColor:=clBlack;
     BaseReport.Rectangle(posx+4,posy+5,posx+200,posy+34);
     BaseReport.PrintXY(posx+5,posy+24,'LISTADO DE HISTORICO DE VALORES POR SERICIO DE TRASLADOS'); // + '  -----> TOTAL ACCIONES: ' + TRIM(LABEL3.Caption));
     Logo:=posy+27;
    end;
   end;
   ...
   begin
    with Rvs1 do begin
     ...
     BaseReport.Rectangle(posx+190,posy,posx+200,posy+5);
     //BaseReport.Rectangle(posx+120,posy,posx+200,posy+5);
     //BaseReport.Rectangle(posx+143,posy,posx+200,posy+5);
     BaseReport.PrintXY(posx+5,posy+4,'TipoTraslado');
     BaseReport.PrintXY(posx+41,posy+4,'Fechavigencia');
     BaseReport.PrintXY(posx+81,posy+4,'ValorKm');
     BaseReport.PrintXY(posx+111,posy+4,'Iva');
     BaseReport.PrintXY(posx+141,posy+4,'ImporteTotal');    //
     //BaseReport.PrintXY(posx+171,posy+4,'ImporteFijo');
     BaseReport.PrintXY(posx+191,posy+4,'Activo');
...
begin
 try
  ...
  for t:=1 to DataSource1.dataset.RecordCount do begin
   ...
   RVS1.BaseReport.PrintLeft(trim(DataSource1.dataset.Fields[2].AsString),posfx+41);
   RVS1.BaseReport.MoveTo(posw,posfy);
   RVS1.BaseReport.PrintLeft('$ ' + trim(DataSource1.dataset.Fields[3].AsString),posfx+81);
   RVS1.BaseReport.MoveTo(posw,posfy);
   ...
   //RVS1.BaseReport.MoveTo(posw,posfy);
   //RVS1.BaseReport.PrintLeft('$ ' + trim(DataSource1.dataset.Fields[7].AsString),posfx+171);
   RVS1.BaseReport.MoveTo(posw,posfy);
   RVS1.BaseReport.PrintLeft(trim(DataSource1.dataset.Fields[4].AsString),posfx+191);
   //RVS1.BaseReport.MoveTo(posw,posfy);
   ...
   RVS1.BaseReport.Rectangle(posfx+190,posfy,posfx+200,posfy-4);
...
procedure TCamValorTraslado.RvS1BeforePrint(Sender: TObject);
begin
Coloca1(2);
end;

procedure TCamValorTraslado.RvS1AfterPrint(Sender: TObject);
begin
Coloca1(1);
end;

procedure TCamValorTraslado.ExportarDatos1Click(Sender: TObject);
