...
procedure TCamValorSepe.FormCreate(Sender: TObject);
begin
gh:=false;
end;

procedure TCamValorSepe.FormActivate(Sender: TObject);
begin
...
procedure TCamValorSepe.Coloca1(nro:byte);
var t,y:integer;
    txt,sec:string;
    txtw:widestring;
...
begin
 if nro=1 then begin
  ...
  +' convert(char(10),Vigencia,103) as Vigencia,Activo FROM TiposServiciosNivel WHERE (NroTipoServicio = 8) ORDER BY IdTipoServicioNivel DESC' ;
 ...
 if nro=2 then begin
  ...
  ParcelasMantenimientoPrecios.ConnectionString:=modulo2.Conexion;
  txtw:='SELECT  IdTipoServicioNivel as id, NroTipoServicio, Nivel, PrecioLista, PrecioContado, PrecioCheque, '
  +' convert(char(10),Vigencia,103) as Vigencia,Activo FROM TiposServiciosNivel WHERE (NroTipoServicio = 8) ORDER BY IdTipoServicioNivel asc' ;
  if not ParcelasMantenimientoPrecios.Listar(txtw,DataSet) then
   messagedlg('ERROR al realizar Transac-SQL. Consulte con Administrador',mtError,[mbok],0);
  dbgrid1.Columns[0].Width:=30;
  dbgrid1.Columns[1].Width:=80;
...
procedure TCamValorSepe.SALEClick(Sender: TObject);
begin
Close;
end;

procedure TCamValorSepe.GUARDAClick(Sender: TObject);
var gf,t:integer;
var m,a:boolean;
var txtw:widestring;
begin
 ...
 if gf = 1 then begin
  PROCESO1.Close;
  PROCESO1.CommandText:='exec SPC_TiposServiciosSepeliosInsertar ' + trim(imp.Text) + ',' + chr(39) + trim(fecha.Text) + chr(39); //trim(cuif.Text) //
  PROCESO1.Open;
  if PROCESO1.Fields[0].AsString = 'OK' then begin
   messagedlg('EXITO al Guardar el VALOR del SEPELIO...',mtConfirmation,[mbok],0);
   Coloca1(1);
   guarda.Enabled :=false;
   grupovalor.Enabled:=false;
  end
  else begin
   messagedlg('Error al Guardar el VALOR del SEPELIO. Consulte con un Administrador',mtError,[mbok],0);
  end;
 end
 else begin
...
procedure TCamValorSepe.JvBitBtn3Click(Sender: TObject);
begin
close;
end;

procedure TCamValorSepe.DBGrid1CellClick(Column: TColumn);
var gf:integer;
begin
...
procedure TCamValorSepe.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TCamValorSepe.FECHAExit(Sender: TObject);
var fec:Tdatetime;
begin
  try
...
procedure TCamValorSepe.FECHAKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TCamValorSepe.impKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then begin
    Key := #0;
    guarda.SetFocus;
...
procedure TCamValorSepe.impExit(Sender: TObject);
begin
 if (trim(imp.Text)='') or (trim(imp.Text)='0') then imp.Text:='00.00';
 if trim(imp.Text)='00.00' then begin
  messagedlg('ERROR!! EL Importe del SEPELIO NO puede ser Cero(0)!!',mterror,[mbok],0);
  imp.SetFocus;
 end;
end;

procedure TCamValorSepe.JvBitBtn1Click(Sender: TObject);
begin
RVS1.Execute;
end;

procedure TCamValorSepe.RvS1Print(Sender: TObject);
...
var sd,sd1,posn,posw:double;
   ...
   begin
    with RvS1 do begin
     ...
     BaseReport.FontColor:=clBlack;
     BaseReport.Rectangle(posx+4,posy+5,posx+200,posy+34);
     BaseReport.PrintXY(posx+5,posy+24,'LISTADO DE HISTORICO DE VALORES POR SERICIO DE SEPELIO'); // + '  -----> TOTAL ACCIONES: ' + TRIM(LABEL3.Caption));
     Logo:=posy+27;
    end;
   end;
...
procedure TCamValorSepe.ExportarDatos1Click(Sender: TObject);
...
procedure TCamValorSepe.RvS1BeforePrint(Sender: TObject);
begin
Coloca1(2);
end;

procedure TCamValorSepe.RvS1AfterPrint(Sender: TObject);
begin
Coloca1(1);
end;
